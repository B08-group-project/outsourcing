...
const Mypage = () => {
    ...
    const { data: coursePlaces, isLoading, isError, error } = useQuery({
        queryKey: ['coursePlaces'],
        queryFn: async () => {
            const { data: { user } } = await supabase.auth.getUser();
            // 코스정보
            const { data: PlaceData, error } = await supabase.from('course').select('id, created_at, course_places(id, course_id, created_at, place_name, address_name, phone)').eq('user_id', user.id);
            // 닉네임
            const { data: userData, error: userError } = await supabase.from('users').select('nickname').eq('id', user.id).single();
            ...
            <div className='flex w-full h-screen'>
                ...
                <div className='flex-[3] flex justify-center items-center'>
                    <div className='flex flex-col w-80 justify-center absolute top-[200px] border-double shadow-xl rounded-2xl border-sky-500 p-8'>
                        ...
                        <label htmlFor='email' className='mb-1'>My Email</label>
                        ...
                        <input type='text' className='w-full h-8 px-2 py-2 mb-5 text-xs border-2 rounded-md' value={nickname} onChange={HandleNicknameChange} />

                        <button className='h-8 p-2 mt-2 text-white rounded text-x7 bg-sky-300' onClick={ChangeNickname}>수정 하기</button>
                    </div>
                </div>
                    ...
                    <div className='flex flex-wrap mx-6'>
                        {coursePlaces.map((courses, placeIndex) => (
                            <div key={placeIndex} className='w-full mb-8'>
                                <h2 className='mx-4 mb-2 text-xl font-bold'>{placeIndex + 1}번째 코스 <span className='text-sm/[16px] text-slate-500'>{courses.created_at}</span></h2>
                                <div className='flex flex-wrap items-center'>
                                    {courses.course_places.map((place, placeIndex) => (
                                        <div key={place.id} className='relative w-[270px] p-4 h-[100px] mx-6 mb-6 shadow-2xl flex flex-col justify-center leading-6 text-sm/[14px]'>                               
